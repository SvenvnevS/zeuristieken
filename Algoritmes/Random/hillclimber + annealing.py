...
def hillclimber(G, iterations):
	...
	for iter in range(iterations):
		...
		old_S = scoreCounter1(old_G, [old_G.nodes[node]['color'] for node in old_G.nodes()])

		# randomly swap a node's color
		G = randomSwap(G)

		# get the new score
		new_S = scoreCounter1(G, [G.nodes[node]['color'] for node in G.nodes()])
...
def scoreCounter(G, colormap, cost_table):
	kleur_labels = {'red': 		0,
					'green': 	1,
					'blue': 	2,
					'yellow': 	3,
